    ...
    try:
        return IOSuccess(
            [
                ...
                .bind(lambda ResultSet: access_to_HTML_general(ResultSet, parner="Carulla"))
                .unwrap()
                for URL in URLS
            ]
        )
...
def main_carulla() -> None:
    read_exel_py("SRC/back_end/web_scrapping/Carulla/carulla_URLs.csv").bind(
        extract_urls
    ).bind(
        unite_index
    )  # Depuración
